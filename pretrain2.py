...
loss_func_char = nn.CrossEntropyLoss(ignore_index=wiki_data.char_dic.get_id('<pad>'))
...
for epoch in range(epoch_num):
    ...
    for i, (source_ids, target_ids) in enumerate(data_loader):
        ...
        source_input = source_ids[:, :-1].contiguous()
        source_label = source_ids[:, 1:].contiguous()
        output, output_char = model(source_input, target_input)
        word_num = output.size(-1)
        char_num = output_char.size(-1)
        loss1 = loss_func(output.view(-1, word_num), target_label.view(-1))
        loss2 = loss_func_char(output_char.view(-1, char_num), source_label.view(-1))
        loss = loss1 + loss2
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def transform(tensor):
    ...
    tensor = torch.where(tensor > 0, torch.tensor([1,],device=tensor.device), torch.tensor([-1,],device=tensor.device))
